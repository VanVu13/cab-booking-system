...
export default function Rating() {
    ...
    const driverAvatar = ride?.driverAvatar
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.success("Cảm ơn đánh giá của bạn!", { toastId: 'rating_success' })
            navigate('/')
        } catch (error: any) {
            if (error?.response?.status === 400 && error?.response?.data?.message?.includes('already rated')) {
                toast.info("Bạn đã đánh giá chuyến đi này rồi.", { toastId: 'rating_already' })
            } else {
                toast.error("Gửi đánh giá thất bại", { toastId: 'rating_fail' })
            }
        ...
        <div className="flex h-full flex-col bg-gray-50 overflow-y-auto">
            <div className="flex-1 flex flex-col items-center justify-center p-6 space-y-6">
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm w-full text-center space-y-2 border border-gray-100">
                    ...
                    <h2 className="text-xl font-bold text-gray-900">Đã thanh toán</h2>
                ...
                <div className="bg-white p-6 rounded-2xl shadow-sm w-full space-y-4 border border-gray-100 flex flex-col items-center">
                    <div className="h-20 w-20 rounded-full bg-gray-200 overflow-hidden flex items-center justify-center border-4 border-white shadow-sm -mt-10">
                        {driverAvatar ? (
                            <img src={driverAvatar} alt="Driver" className="h-full w-full object-cover" />
                        ) : (
                    ...
                    <div className="text-center">
                        ...
                        <p className="text-gray-500">Bạn thấy chuyến đi thế nào?</p>
                    </div>
                    ...
                    <div className="w-full mt-4">
                        <textarea
                            ...
                            rows={3}
                            placeholder="Để lại lời khen hoặc góp ý..."
                            value={comment}
